...
#            a = 0
        ...
        self.retriever_texts = BM25Retriever(document_store=document_store_texts)    


        document_store_tables = ElasticsearchDocumentStore(host=host, username="", password="", index="tables")
        
        self.retriever_tables = BM25Retriever(document_store=document_store_tables)  
        
        self.sentence_model = SentenceTransformer('sentence-transformers/multi-qa-mpnet-base-dot-v1')
    ...
    def retrievalTexts(self):
        if self.count_steps==0:
            ...
            for doc in candidate_documents:
                ...
                self.groups.append([{'id':doc.id, 'type': 'text', "content":doc.content, "embedding":self.sentence_model.encode(doc.content), "title":doc.meta["title"]}])
            ...
            for i in range(len(self.groups)):
                ...
                for j in range(len(self.groups[i])):
                    if self.groups[i][j]["type"] == "text":
                        ...
                        input_string = input_string + " " + self.groups[i][j]["title"] + ' '  + self.groups[i][j]["content"]
                input_strings.append(' '.join(input_string.split()[:1024]))

            candidate_documents_list = self.retriever_texts.retrieve_batch(
                                    queries=input_strings,
                                    top_k=self.number_passages_group_retrieval+text_count,
                                    )
            for i in range(len(candidate_documents_list)):
                candidate_documents = candidate_documents_list[i]
                for doc in candidate_documents:
                    flag_repeated = False
                    for j in range(len(self.groups[i])):
                    ...
                    if not flag_repeated:
                        self.groups[i].append({'id':doc.id, 'type': 'text', "content":doc.content, "embedding":self.sentence_model.encode(doc.content), "title":doc.meta["title"]})
                    flag_repeated = False
                    for text in self.texts:
                        if text['id'] == doc.id:
                            ...
                            break
                    if not flag_repeated:
                        self.texts.append({'id':doc.id, "content":doc.content, "embedding":self.sentence_model.encode(doc.content), "title":doc.meta["title"]})
    ...
    def retrievalTables(self):
        ...
        if self.count_steps==0:
            ...
            for doc in candidate_documents:
                self.tables.append({'id':doc.id, "content":doc.content, "embedding":self.sentence_model.encode(doc.content), "title":doc.meta["title"]})
                self.groups.append([{'id':doc.id, 'type': 'table', "content":doc.content, "embedding":self.sentence_model.encode(doc.content), "title":doc.meta["title"]}])
            ...
            for i in range(len(self.groups)):
                ...
                for j in range(len(self.groups[i])):
                    if self.groups[i][j]["type"] == "text":
                        ...
                        tables_count += 1
                input_strings.append(' '.join(input_string.split()[:1024]))
            candidate_documents_list = self.retriever_tables.retrieve_batch(
                                    queries=input_strings,
                    ...
                    for j in range(len(self.groups[i])):
                        for passage in self.groups[i]:
                            if passage['id'] == doc.id:
                                ...
                                break
                    if not flag_repeated:
                        self.groups[i].append({'id':doc.id, 'type': 'table', "content":doc.content, "embedding":self.sentence_model.encode(doc.content), "title":doc.meta["title"]})
                    flag_repeated = False
                    for tables in self.tables:
                        if tables['id'] == doc.id:
                            if tables['content'] == doc.content:
                                flag_repeated=True
                            break
                    if not flag_repeated:
                        self.tables.append({'id':doc.id, "content":doc.content, "embedding":self.sentence_model.encode(doc.content), "title":doc.meta["title"]})
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.dataset == "train":
            ...
            self.question = {"question": question_text, "embedding": self.sentence_model.encode(question_text)}
        else: 
        ...
        self.done = False
        self.count_steps = 0
        self.observation = np.concatenate((self.question["embedding"], np.zeros(10*768)))
        info = {}

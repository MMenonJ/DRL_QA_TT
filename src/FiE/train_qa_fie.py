...
import sys
from DPR.dpr.models.optimization import AdamWLayer
from DPR.dpr.utils.dist_utils import all_gather_list
from FiE.evaluate_fie import f1_score, exact_match_score, update_sp
...
def common_args():
    ...
    parser.add_argument("--max_ans_len", default=15, type=int)
...
def train_args():
    ...
    parser.add_argument("--num_ctx", type=int, default=50)
...
#     eval_dataset = QADatasetNoSP(tokenizer, json.load(open(args.predict_file, 'r')), args.max_seq_len, args.max_q_len, args.max_ans_len, world_size=args.world_size, global_rank=args.global_rank, neg_num=args.neg_num, debug=args.debug, num_ctx=args.num_ctx)
...
#     metrics = pred_func(args, model, eval_dataloader, tokenizer, fixed_thresh=None)
#     print(metrics['f1'])
#     print(metrics['f1'])


def predict_no_sp(args, model, eval_dataloader, tokenizer, fixed_thresh=None):
    model.eval()
    id2answer = {}
    id2gold = {}
    total = 0
    ...
    for batch in eval_dataloader:
        ...
        for qid, ans, offset, text, mappings in zip(batch['qids'], batch['gold_answer'], batch['para_offsets'], batch['original_text'], batch['offset_mapping']):
            ...
            print("Predicted Answer: '" + str(final_ans)+ "'")	
